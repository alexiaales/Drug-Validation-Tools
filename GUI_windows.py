...
def comma():
   messagebox.showinfo( "CLUSTERING TOOL", "You have selected to proceed with the clustering tool") 
   messagebox.showinfo( "CLUSTERING TOOL", "Please insert the dataset you want to perform clustering on.")
...
#------------------------------------------------------------------------------
    ...
    agglom = AgglomerativeClustering(n_clusters=int(w.get()), linkage="ward").fit(X)
    m=kmeans.labels_
    n= agglom.labels_
...
def comme():
   messagebox.showinfo( "CLASSIFICATION TOOL", "You have selected to proceed with the classification tool") 
   ...
   filename = filedialog.askopenfilename()
   messagebox.showinfo( "CLASSIFICATION TOOL", "Thank you,insert the data you want to classify.")
   ...
   classi=['Random Forests','SVM','nLog','SVC-linear','DecisionTree','MPL','AdaBoost','GaussianNB','NN']
   accur=[accuracy1,accuracy2,accuracy3,accuracysvc,accuracydtc,accuracympl,accuracyabc,accuracybnb,accuracy]
   ...
   button = Button(root, text="CLICK HERE TO VIEW ACCURACIES", command=printSomething) 
   button.pack()
   root.mainloop()
   root = tk.Tk()
   v = tk.IntVar()
   ...
   def ShowChoice():
       ...
       if v.get() == 0 or v.get() == 10:
           ...
           teliko=pd.DataFrame(list(zip(name,tava)),columns=['GENES','TARGET VALIDATION '])
           teliko.to_excel('output_rf.xlsx', engine='xlsxwriter')
           messagebox.showinfo( "CLASSIFICATION TOOL", "You are ready to see your results!")
       ...
       if v.get() == 10 :
           ...
           outputs=['output_adaboost.xlsx','output_rf.xlsx','output_svm.xlsx','output_lg.xlsx','output_knn.xlsx','output_dtc.xlsx','output_svc.xlsx','output_mpl.xlsx','output_bnb.xlsx']
